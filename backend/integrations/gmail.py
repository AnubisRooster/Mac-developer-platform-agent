...
class GmailIntegration:
        ...
        if os.path.exists(self._token_path):
            try:
                ...
                self._logger.warning(
                    "Failed to load token file %s: %s", self._token_path, e
                )
        if not creds or not creds.valid:
            ...
            if not creds or not creds.valid:
                if not os.path.exists(self._credentials_path):
                    self._logger.warning(
                        "Gmail OAuth not configured: credentials file %s not found.",
                        self._credentials_path,
                    )
                    return None
                self._logger.warning(
                    "Gmail OAuth flow needed: token file %s missing or invalid.",
                    self._token_path,
                )
                return None
        try:
    ...
    def summarize_thread(self, thread_id: str) -> dict[str, Any]:
        ...
        if messages_data:
            first_headers = {
                h["name"].lower(): h["value"]
                for h in thread["messages"][0].get("payload", {}).get("headers", [])
            }
