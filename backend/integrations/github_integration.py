...
logger = logging.getLogger("claw-agent.integrations.github")
...
class GitHubIntegration:
    ...
    def create_branch(
        ...
        self._logger.info(
            "Creating branch %s from %s in %s", branch_name, from_branch, repo
        )
        repository = self._client.get_repo(repo)
    ...
    def get_repo_activity(self, repo: str, days: int = 1) -> dict[str, Any]:
        self._logger.info(
            "Fetching repo activity for %s (last %s days)", repo, days
        )
        repository = self._client.get_repo(repo)
        ...
        for issue in repository.get_issues(state="all"):
            if (
                issue.updated_at
                and issue.updated_at.replace(tzinfo=timezone.utc) >= since
