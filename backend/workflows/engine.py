...
class WorkflowEngine:
    ...
    def __init__(
        self, bus: EventBus | None = None, workflow_dir: str = "workflows"
    ) -> None:
    ...
    def load(self) -> None:
        ...
        logger.info(
            "WorkflowEngine ready — %d workflow(s) registered",
            len(self._workflows),
        )
    ...
    def get_workflows(self) -> list[dict[str, Any]]:
        """Return all workflow definitions as dicts for the API."""
        return [wf.model_dump() for wf in self._workflows.values()]
    ...
    async def run_workflow(
        ...
        for i, action in enumerate(wf.actions):
            step_label = (
                f"[{wf.name} step {i + 1}/{len(wf.actions)}] "
                f"{action.description or action.tool}"
            )
            ...
            if not tool_func:
                ...
                results.append(
                    {"step": i + 1, "tool": action.tool, "error": "tool_not_found"}
                )
            ...
            try:
                ...
                results.append(
                    {"step": i + 1, "tool": action.tool, "error": str(exc)}
                )
                if action.on_failure == "stop":
                    status = "failed"
                    break
