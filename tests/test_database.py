...
class TestCachedSummaryModel:
    def test_create_summary(self, db_session):
        summary = CachedSummary(key="pr:org/repo:42", summary="This PR adds X.")
        db_session.add(summary)
        db_session.commit()

        result = db_session.query(CachedSummary).first()
        assert result.key == "pr:org/repo:42"
        assert result.summary == "This PR adds X."

    def test_unique_key_constraint(self, db_session):
        db_session.add(CachedSummary(key="unique-key", summary="First"))
        db_session.commit()
        db_session.add(CachedSummary(key="unique-key", summary="Duplicate"))
